...
class EventPipeline:
    ...
    def PushEvent(self, method_name, args, time_stamp, vol=False):
        ...
        if not  isinstance(args, collections.Iterable):
            method_name=[args]
        event = {'method': method_name,'args':args,'time_stamp':time_stamp,'volatile':vol}
    ...
    def InsertEvent(self, method_name, args, time_stamp, vol=False, loc = 'head'):
        if loc == 'head':
            idx = next((i for i,e in enumerate(self.event_list) if int(e['time_stamp']) >= int(time_stamp)), self.eventcount)
        if loc == 'tail':
            idx = next(i for i,e in enumerate(self.event_list) if e['time_stamp'] >= time_stamp)
        event = {'method': method_name,'args':args,'time_stamp':time_stamp,'volatile':vol}
