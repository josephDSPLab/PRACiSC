...
class SpectrumAnalyzer:
    ...
    def __init__(self):
        ...
        self.test.Live_event()           # Online random playing event determined by prosperity function
        self.test.set_tempo_pattern(4, 4) # if the meta file is exist, calling this routine is not required
        if using_callback:
            ...
            while self.stream.is_active():
                if len(self.buffer) == self.RATE * 14:
                    ...
                    t.start()
                    print(int(tmp3_1))
                    self.test.StartInTime(np.mean(np.diff(tmp3_2)) * 4 - (14-tmp3_2[-1]) - t_proc, int(tmp3_1))
                    break
